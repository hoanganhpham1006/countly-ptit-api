...
class DevicesHelper(object):
    ...
    def get_devices(self, number, start_time, end_time, page):
        ...
        if start_time > end_time:
          start_time = end_time
        return self.get_devices_process(json_data, number, page, start_time, end_time)

    def get_devices_process(self, json_data, number, page, start_time, end_time):
        total_dict = {}
                        ...
                        for month in json_data[year]:
                            if str.isdigit(month):
                                for day in json_data[year][month]:
                                    if str.isdigit(day):
                                        for device in json_data[year][month][day]:
                                            dt = time.mktime(datetime.datetime(int(year), int(month), int(day), 12, 0, 0).timetuple())
                                            if dt >= start_time and dt <= end_time:
                                                if device in json_data['meta']['os']:
                                                    if 't' in json_data[year][month][day][device]:
                                                        if device in total_dict:
                                                            total_dict[device] += json_data[year][month][day][device]['t']
                                                        else:
                                                            total_dict[device] = json_data[year][month][day][device]['t']
